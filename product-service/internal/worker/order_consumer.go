...
func (w *OrderWorker) ListenForOrders(ctx context.Context) {
	...
	for {
		select {
		case <-ctx.Done():
			logger.Log.Info("Gracefully stopping order worker")
			return
		default:
			...
			if err != nil {
				...
				logger.Log.Error("failed to read order stream", zap.Error(err))
				continue
			}
			...
			for _, stream := range entries {
				...
				for _, msg := range stream.Messages {
					...
					if !ok || itemsStr == "" {
						logger.Log.Warn("dropping invalid order message: missing items", zap.Any("raw_values", msg.Values))
						if _, err := w.brokerRedis.XAck(msgCtx, STREAM_NAME, GROUP_NAME, msg.ID).Result(); err != nil {
					...
					if !ok || orderIDStr == "" {
						logger.Log.Warn("dropping invalid order message: missing order_id", zap.Any("raw_values", msg.Values))
						if _, err := w.brokerRedis.XAck(msgCtx, STREAM_NAME, GROUP_NAME, msg.ID).Result(); err != nil {
							logger.Log.Error("failed to acknowledge invalid message", zap.String("msgID", msg.ID), zap.Error(err))
						}
						continue
					...
					if _, err := fmt.Sscanf(orderIDStr, "%d", &orderID); err != nil {
						logger.Log.Warn("dropping invalid order message: invalid order_id",
					...
					if err := json.Unmarshal([]byte(itemsStr), &items); err != nil {
						logger.Log.Warn("dropping invalid order message: malformed items",
							zap.String("msgID", msg.ID),
							zap.Error(err),
						)
					...
					err = w.service.ReserveStock(msgCtx, orderID, stockUpdates)
					if err != nil {
						logger.Log.Error("failed to process order message", zap.String("msgID", msg.ID), zap.Error(err))
						continue // Do not ack the message, so it can be retried
					}

					// If processing is successful, acknowledge the message
					if _, err := w.brokerRedis.XAck(msgCtx, STREAM_NAME, GROUP_NAME, msg.ID).Result(); err != nil {
						logger.Log.Error("failed to acknowledge processed order message", zap.String("msgID", msg.ID), zap.Error(err))
						continue
					}
					logger.Log.Info("Order message processed successfully", zap.String("msgID", msg.ID))
				}
			}
		}
	}
...
func (w *OrderWorker) moveToDLQ(ctx context.Context, msg redis.XMessage) error {
	return infrastructure.MoveToDLQ(ctx, w.brokerRedis, msg, "stream:orders:created:dlq", "Exceeded max retries (5)")
}
