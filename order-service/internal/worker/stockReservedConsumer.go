...
func NewStockreservedWorker(brokerRedis *redis.Client, service *service.OrderService) *StockreservedWorker {
	return &StockreservedWorker{brokerRedis: brokerRedis, service: service}
}

func (w *StockreservedWorker) ListenForStockReserved(ctx context.Context) {
	...
	for {
		...
		default:
			entries, err := w.brokerRedis.XReadGroup(ctx, &redis.XReadGroupArgs{
				...
				Streams:  []string{"stream:stock:reserved", currentID},
				Count:    1,
				Block:    5000,
			}).Result()
			...
			for _, stream := range entries {
				...
				for _, msg := range stream.Messages {
					...
					if len(pendingInfo) > 0 && pendingInfo[0].RetryCount >= 5 {
						...
						w.moveToDLQ(ctx, msg)
						_, _ = w.brokerRedis.XAck(ctx, "stream:stock:reserved", "order-group", msg.ID).Result()
						continue
					}
					...
					// Process the stock reserved event
					err := w.service.ProcessAwaitingPaymentOrders(ctx, orderIDStr)
					if err != nil {
						continue
					}
					// Acknowledge the message after processing
					_, err = w.brokerRedis.XAck(ctx, "stream:stock:reserved", "order-group", msg.ID).Result()
					if err != nil {
						continue
					}
...
func (w *StockreservedWorker) moveToDLQ(ctx context.Context, msg redis.XMessage) {
    infrastructure.MoveToDLQ(ctx, w.brokerRedis, msg, "stream:stock:reserved:dlq", "Exceeded max retries (5)")
}
