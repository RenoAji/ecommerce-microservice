...
func (w *PaymentFailedWorker) ListenForPaymentFailed(ctx context.Context){
	...
	for {
		...
		default:
			entries, err := w.brokerRedis.XReadGroup(ctx, &redis.XReadGroupArgs{
				...
				Streams:  []string{"stream:payment:failed", currentID},
				Count:    1,
				Block:    5000,
			}).Result()
			...
			for _, stream := range entries {
				...
				for _, msg := range stream.Messages {
					...
					if len(pendingInfo) > 0 && pendingInfo[0].RetryCount >= 5 {
						...
						w.moveToDLQ(ctx, msg)
						_, _ = w.brokerRedis.XAck(ctx, "stream:payment:failed", "order-group", msg.ID).Result()
						continue
					}
					...
					_, err = w.brokerRedis.XAck(ctx, "stream:payment:failed", "order-group", msg.ID).Result()
					if err != nil {
						continue
					}
...
func (w *PaymentFailedWorker) moveToDLQ(ctx context.Context, msg redis.XMessage) {
