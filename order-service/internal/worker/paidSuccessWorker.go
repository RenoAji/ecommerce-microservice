	...
	for {
		...
		default:
			entries, err := w.brokerRedis.XReadGroup(ctx, &redis.XReadGroupArgs{
				...
				Streams:  []string{"stream:payment:success", currentID},
				Count:    1,
				Block:    5000,
			}).Result()
			...
			for _, stream := range entries {
				...
				for _, msg := range stream.Messages {
					...
					if len(pendingInfo) > 0 && pendingInfo[0].RetryCount >= 5 {
						...
						w.moveToDLQ(ctx, msg)
						_, _ = w.brokerRedis.XAck(ctx, "stream:payment:success", "order-group", msg.ID).Result()
						continue
					}
					...
					err := w.service.UpdateOrderStatus(ctx, orderIDStr, "PAID")
					if err != nil {
						continue
					}
					// Acknowledge the message after processing
					_, err = w.brokerRedis.XAck(ctx, "stream:payment:success", "order-group", msg.ID).Result()
					if err != nil {
						continue
					}
